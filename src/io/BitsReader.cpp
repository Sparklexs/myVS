...
BitsReader::BitsReader(const uint32_t *in, uint64_t len) :
		in_(in), term_(in + len), buffer_(0), fill_(0) {
	if (in == NULL)
...
uint32_t BitsReader::read_bits(uint32_t num) {
	...
	BITSRD_BUFFILL(num);
	fill_ -= num;
	return (buffer_ >> fill_) & ((uint64_t(1) << num) - 1);
}
...
// 利用表查找来快速读取1元编码的值，这里采用的是16bit为单位查找
...
// gamma编码值的unary编码部分使用表查找，其余部分直接读取
...
void BitsReader::ngammaArray(uint32_t *out, uint64_t nvalues) {
	if (out == NULL)
	...
	for (uint64_t i = 0; i < nvalues && in_ <= term_; i++)
		out[i] = read_ngamma();
}

void BitsReader::fgammaArray(uint32_t *out, uint64_t nvalues) {
	if (out == NULL)
		THROW_ENCODING_EXCEPTION("Invalid value: out");
	if (nvalues == 0)
		THROW_ENCODING_EXCEPTION("Invalid value: nvalues");
...
void BitsReader::fugammaArray(uint32_t *out, uint64_t nvalues) {
	if (out == NULL)
		THROW_ENCODING_EXCEPTION("Invalid value: out");
	if (nvalues == 0)
		THROW_ENCODING_EXCEPTION("Invalid value: nvalues");
...
// 表查找delta编码值的gamma编码部分
uint32_t BitsReader::read_fgdelta() {
	uint32_t count = read_fgamma();
	ASSERT(count < 32);
	return ((1U << count) | read_bits(count)) - 1;
}
// delta编码值的unary编码部分使用表查找，区域部分直接读取
...
void BitsReader::ndeltaArray(uint32_t *out, uint64_t nvalues) {
	if (out == NULL)
		THROW_ENCODING_EXCEPTION("Invalid value: out");
	if (nvalues == 0)
		THROW_ENCODING_EXCEPTION("Invalid value: nvalues");
...
void BitsReader::fudeltaArray(uint32_t *out, uint64_t nvalues) {
	if (out == NULL)
		THROW_ENCODING_EXCEPTION("Invalid value: out");
	if (nvalues == 0)
		THROW_ENCODING_EXCEPTION("Invalid value: nvalues");
...
void BitsReader::fgdeltaArray(uint32_t *out, uint64_t nvalues) {
	if (out == NULL)
		THROW_ENCODING_EXCEPTION("Invalid value: out");
	if (nvalues == 0)
		THROW_ENCODING_EXCEPTION("Invalid value: nvalues");
...
void BitsReader::fdeltaArray(uint32_t *out, uint64_t nvalues) {
	if (out == NULL)
		THROW_ENCODING_EXCEPTION("Invalid value: out");
	if (nvalues == 0)
		THROW_ENCODING_EXCEPTION("Invalid value: nvalues");
...
void BitsReader::intrpolatvArray(uint32_t *out, uint32_t nvalues,
		uint32_t offset, uint32_t low, uint32_t high) {
	...
	uint32_t m = read_intrpolatv(high - nvalues + h + 1 - (low + h) + 1) + low
			+ h;
