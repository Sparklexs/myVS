...
/*
 * size指向的位置存储filename文件的大小(in Byte)
 */
uint32_t *OpenFile(const std::string& filename, uint64_t *size) {
	void *addr = NULL;
...
void FastMemcpy(void *dest, void *src, uint64_t n) {
	if (n == 0)
		return;
