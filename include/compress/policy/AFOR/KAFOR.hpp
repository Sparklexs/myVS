...
#include <compress/policy/AFOR/KAFOR_pack.hpp>
#include <compress/policy/AFOR/KAFOR_unpack.hpp>

namespace integer_encoding {
namespace internals {

class KAFOR: public Compressor, public EncodingBase {
public:
...
private:
	template<typename T>
	int encodeBlock(char* des, const T* src, uint32_t encodeNum) const;

	template<typename T>
	int decodeBlock(T* des, const char* src, uint32_t decodeNum) const;

	template<typename T>
	uint32_t decodeAllFrames(T* des, const char* src,
			const char* frameHeaderArr, uint32_t numFrames) const;

	template<typename T>
	uint32_t chooseAndSaveBestConfig(char *des, const T *maxQuadArr,
			uint32_t maxQuadNum, KAFORConfig tmpConfigArr[]) const;

	template<typename T>
	uint32_t chooseBestConfig(const T *src,
			KAFOR::KAFORConfig configArr[]) const;

	template<typename T>
	uint32_t encodeAllFrames(char *des, const T *src,
			const char* frameHeaderArr, uint32_t numFrames) const;
...
template<typename T>
int KAFOR::encode(char* des, const T* src, uint32_t encodeNum) const {
	uint32_t compLen = 0;
	uint32_t encoded = 0;
	while (encoded < encodeNum) {
		if (encoded + m_windowSize <= encodeNum) {
			int alignedNumInt = encodeNum / 128 * 128; //这个赋值应该可以移至循环外部
			compLen += encodeBlock(des + compLen, src + encoded, alignedNumInt);
			encoded += alignedNumInt;
		} else {
			compLen += (uint32_t) VarByte::encode(des + compLen, src + encoded,
					encodeNum - encoded);
			encoded = encodeNum;
		}
	}
...
template<typename T>
int KAFOR::decode(T* des, const char* src, uint32_t decodeNum) const {
	uint32_t byteDecompressed = 0;
	uint32_t decoded = 0;
	while (decoded < decodeNum) {
		if (decoded + m_windowSize <= decodeNum) {
			int alignedNumInt = decodeNum / 128 * 128;
			byteDecompressed += decodeBlock(des + decoded,
					src + byteDecompressed, alignedNumInt);
...
template<typename T>
int KAFOR::encodeBlock(char* des, const T* src, uint32_t encodeNum) const {
	...
	uint32_t totalNumFrameHeader = chooseAndSaveBestConfig(des, maxQuadIntArr,
			k, configArr);
	// 注意des作为指针参数，其指向的值可以被改变，而des本身存储的地址不变
	// 因此frameHeaderArr仍指向des+4的位置，用于接下来读取每个header中的ID
	// 而des则需要从4+frameHeaderArr之后的位置开始压缩所有元素
	...
	*desInt = 4 + totalNumFrameHeader;// record the begin position of data segment
	...
	int compLen = encodeAllFrames(des, src, frameHeaderArr,
			totalNumFrameHeader);
	des += compLen;
...
template<typename T>
int KAFOR::decodeBlock(T* des, const char* src, uint32_t decodeNum) const {
	...
	uint32_t dataSegByte = decodeAllFrames(des, dataSegPos, frameHeaderArr,
			numFrames);
	return dataSegPos + dataSegByte - src;
}

template<typename T>
uint32_t KAFOR::chooseAndSaveBestConfig(char *des, const T *maxQuadArr,
		uint32_t maxQuadNum, KAFORConfig tmpConfigArr[]) const {
	...
	for (int i = 0; i < maxQuadNum; i += 32) {
		...
		totalNumFrameHeader += configChoosen.numFrames;	//one 1-Byte header for each frame storing ID
		// save frame header
		for (int j = 0; j < configChoosen.numFrames; ++j) {
			uint8_t frameHeader = configChoosen.frames[j].frameLengthId
					+ configChoosen.frames[j].bitWidth - 1;	// bitwidth -> 0 to 31
			*des++ = frameHeader;
		}
		maxQuadArr += 32;
	}
...
template<typename T>
uint32_t KAFOR::decodeAllFrames(T* des, const char* src,
		const char* frameHeaderArr, uint32_t numFrames) const {
	...
	for (uint32_t i = 0; i < numFrames; ++i) {
		...
		if (KAFORUnpackInfoIdx >= 384)
			std::cout << "wrong KAFORUnpackInfoIdx!" << KAFORUnpackInfoIdx
					<< std::endl;

		const KAFORUnpackInfo &info = KAFORUnpackInfoArr[KAFORUnpackInfoIdx];
		if (byteOffset << 3 != info.m_offset)
			std::cout << "wrong info offset!" << (byteOffset << 3) << " "
					<< info.m_offset << std::endl;
...
template<typename T>
uint32_t KAFOR::chooseBestConfig(const T *src,
		KAFOR::KAFORConfig configArr[]) const {
	...
	for (uint32_t i = 0; i < 6; ++i) {
		...
		for (uint32_t j = 0; j < configArr[i].numFrames; ++j) {
			curSize += configArr[i].frames[j].bitWidth
					* configArr[i].frames[j].frameLength * 4;//do not forget k=4
		}
...
template<typename T>
uint32_t KAFOR::encodeAllFrames(char *des, const T *src,
		const char* frameHeaderArr, uint32_t numFrames) const {
	uint32_t *desInt = (uint32_t*) des;
	uint32_t byteOffset = 0;	// 0 to 3
	for (uint32_t i = 0; i < numFrames; ++i) {
