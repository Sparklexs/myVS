...
class VSEncodingDP {
public:
	explicit VSEncodingDP(const uint32_t *lens, const uint32_t *zlens,
...
private:
	bool aligned_;
	uint32_t size_; //输入序列长度
	uint32_t mxblk_; //最大分块长度
	const uint32_t *lens_; //指向一组可选的分块长度数组
	const uint32_t *zlens_; //只在VSE_Blocks中使用了，用处暂时不明了
