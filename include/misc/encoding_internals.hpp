 ...
 */
inline void MEMCPY(void *dest, void *src, uint64_t n) {
	ASSERT(dest != NULL);
	ASSERT(src != NULL);
...
public:
	explicit encoding_exception(const std::string& msg) :
			msg_(msg), file_(NULL), func_(NULL), line_(0) {
	}

	explicit encoding_exception(const std::string& msg, const char *file,
			const char *func, int line) :
			msg_(msg), file_(file), func_(func), line_(line) {
		char buf[256];
		int n = snprintf(buf, 256, "(src:%s func:%s line:%d)", file_.c_str(),
				func_.c_str(), line_);
